...
subregions, see [this vignette](project_subregions.html)). 
...
illustrate, the example population data in `propop` are from 31. December 2023.
...
groups (e.g., proportion of female to male newborns). `propop` uses FSO's
standard values as default option for these arguments.
...
ranging from 0 to 100 years (incl. those who are older).   
...
female, and the share of babies born by mothers who are not Swiss are defined in `propop::propop()`. We use FSO's standard
values as default for these arguments.

If the argument `scenarios` is not specified, `propop::propop()` returns a 
projection for all scenarios available in `parameters`.
\
...
who acquire the Swiss citizenship (`acq_n`) are added if the demographic 
