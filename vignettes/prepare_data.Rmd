...
devtools::load_all() # to run population projection
```
...
```{r get meta data, echo= T, eval=T,  message=FALSE}
...
```{r filter dimensions, echo = T, eval = TRUE, results='hide', message=FALSE, warning=FALSE}
...
dim4_103 <- metadata_tidy |>
  ...
  mutate(values = as.character(0:31))
...
```{r query, eval = TRUE, echo = TRUE, message=FALSE}
# reference scenario
...
```{r process, eval = TRUE, echo = FALSE}
...
```{r get meta data2, echo= T, eval=T,  message=FALSE, warning=FALSE}
...
```{r filter dimensions2, echo = T, eval = TRUE, results='hide', message=FALSE, warning=FALSE}
# Specify the elements to download
dim1 <- metadata_tidy |>
  dplyr::filter(
    text == "Kanton" & # Canton
...
```{r query2, eval = TRUE, message=FALSE,results='hide'}
...
```{r process query2, eval = TRUE, message=FALSE}
# Bring variable names and factor levels into the format required later
...
```{r quick-check-rates, echo = F, eval=FALSE}
# check quickly if figures correspond to STAAG input table for projection
fso_rates |>
  dplyr::filter(
    year == 2024 &
      scen == "reference" &
...
```{r get meta data3, echo= T, eval=T,  message=FALSE, warning=FALSE}
...
```{r filter dimensions3, echo = T, eval = TRUE, results='hide', message=FALSE, warning=FALSE}
# Specify the elements to download
dim1 <- metadata_tidy |>
  dplyr::filter(
    text == "Kanton" & # Canton
...
```{r query3, eval = TRUE, message=FALSE,results='hide'}
# Download rate parameters
...
```{r process query3, eval = TRUE, message=FALSE}
# Bring variable names and factor levels into the format required later
# Process data
...
```{r all in one, echo = T, eval=TRUE, message=F}
...
Show parameters for one demographic group:
...
In addition to the parameters, the projection function also requires a starting
population. To prepare the corresponding query, we again start with the meta data:

```{r get meta data population, echo= T, eval=T, results='hide', message=FALSE, warning=FALSE}
...
```{r filter dimensions 3, echo = T, eval = TRUE, results='hide', message=FALSE, warning=FALSE}
# Specify the elements to download
...
```{r get pop, eval = TRUE, message=FALSE}
# Download population
...
We now process th data to ensure that the population data conforms to 
...
results_raw <- project_raw(
  ...
  share_born_female = 100 / 205,
  subregional = FALSE,
  n = starting_population |> dplyr::pull(n)
)
