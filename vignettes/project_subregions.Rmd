...
```{r data-population}
# Generate 5 random shares, avoiding very uneven distributions
share_1 <- {set.seed(1); round(runif(1, min = 0, max = 0.7), digits = 2)}
share_2 <- {set.seed(2); round(runif(1, min = 0, max = 0.7), digits = 2)}
share_3 <- {set.seed(3); round(runif(1, min = 0, max = 0.7), digits = 2)}
share_4 <- {set.seed(4); round(runif(1, min = 0, max = 0.7), digits = 2)}
...
Let's do this with migration from other countries (`imm_int_n`). To achieve this, 
we first join the dataset containing the projection parameters (`fso_parameters_sub`) 
and the dataset containing the shares (`df_population_shares`). Identifiers for 
demographic groups must be present in both datasets. The actual distribution 
involves only a single line in which the canton-wide number of immigrants 
(`imm_int_n`) is multiplied by the share (`share`).
...
data_distr_pop_size <- fso_parameters_sub |> 
  ...
  dplyr::mutate(imm_int_n_distr = imm_int_n * share) 
```

In the table below, the blue columns `check` and `difference` show that the sum 
of the distributed parameter (`imm_int_n_distr`) adds up to the total number of 
people (`imm_int_n`; the figures provided by the FSO for the whole canton). 

```{r distr-pop-size-check}
data_distr_pop_size |> 
  dplyr::mutate(check = round(sum(imm_int_n_distr), 0), .by = c(year, nat, sex, age)) |> 
  dplyr::filter(sex == "m" & nat == "int") |>
  dplyr::mutate(across(c("share", "imm_int_n_distr"), \(x) round(x, 3))) |> 
...
To proceed with the projection, the column with the distributed international 
immigration (`imm_int_n_distr`) needs to become the new `imm_int`. Otherwise the
parameter won't be recognized by `propop()`. 
...
dataset with five regions. For pedagogical reasons, we modify the data a little 
...
df_hist_imm_int <- tibble::tibble(
  ...
  dplyr::mutate(
    ...
    # historical occurrences for international immigration
...
To use this function, you need to provide a dataset containing a column with
...
fso_parameters_sub_distr_hist |> 
  head(100) |> 
  dplyr::mutate(across(share:imm_int_n_distr, \(x) sprintf(fmt = "%.3f", x))) |>
  DT::datatable()
...
<!--     # TODO prepare imm_nat_n -->
<!--     imm_nat_n = imm_int_n_distr, -->
