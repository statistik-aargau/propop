...
spatial entity (e.g., municipalities within a canton). It discusses 
**two key challenges** that arise when conducting such projections and presents 
...
when you need to downscale input data expressed as **"number of people"**. To 
deal with these challenges, consider the following options:   

- To adjust the cantonal "number of people" estimates among subregions, check the 
next two subsections.  
- To adjust birth rates, you may find the package [`propopbirth`](https://github.com/statistik-aargau/propopbirth) useful.   
- To adjust other rates, the procedure described in section [Migration between subregions](#between-subregions) could be useful.  
...
Let's look at a concrete, numeric example. The column `spatial_unit` in the 
data set `ag_population_subregional` (included in the package) contains the 
population for five subregions at the end of 2025. 
...
and numbers for the subregions are identical to those for the whole canton).
...
line per parameter in which the canton-wide number of immigrants (`imm_int_n` and 
...
To illustrate this approach, we use data included in the package.   
...
<span style="color: #96D4FF;">historical sum of the requested demographic group
and years per spatial unit</span> (`sum_imm_n`) and 
<span style="color: #007AB8;">across all regions</span> (`total_imm_n`).
On this basis,
<span style="color: #005D91;">the proportion of people that should be allocated to the respective 
group and spatial unit </span> is calculated (`imm_share =  imm_n / sum_imm_n`). 
...
Using the same sample group as above, it can be seen that the `group_sum_int` 
...
fso_parameters_sub_distr_hist |>
  ...
  dplyr::mutate(share_imm_int = round(share_imm_int, 3),
                imm_int_n_distr = round(imm_int_n_distr, 3)
                ) |>
  dplyr::arrange(year, age, sex, nat) |> 
  dplyr::filter(sex == "m" & nat == "int" & age == "29") |>
...
- net migration numbers (`subregional = net`) 
- or migration rates (`subregional = rate`).  

For both approaches, extra columns in the parameter 
...
of these annual rates.
For example, if in year 1 two out of 10 people moved away and 
in year 2 three out of 10 people moved away, the estimate would be 
(0.2 + 0.3) / 2 = 0.25.
...
When running `propop()`, the rate‑based method re-allocates individuals each year 
...
since the number of newborns is zero at the beginning of the year, the number 
of births per years is used as a basis for this group).   
1. These emigrants are added up across subregions and put into a "distribution pool".  
1. The people in this pool are then distributed among the subregions using `imm_sub`. 
...
emi_rate |> 
  ...
  dplyr::select(-births, -n_jan)
```

We then use `calculate_shares()` to compute the proportion of people that should 
be allocated to the requested demographic groups and spatial units based on past
migration patterns (see also section on 
...
the parameters (let's re-use `fso_parameters_sub_distr_hist`). 
...
First, we calculate the historical shares of past immigration and emigration.

```{r, eval = TRUE}
# Historic subregional immigration
...
# Historic subregional emigration
...
Then, we compute the net migration by combining the historical shares and 
subtracting the aggregated subregional emigration (`hist_emi_sub_n`) from 
...
subregional_net_migration <- dplyr::full_join(
  subregional_imm_share |> 
    dplyr::select(
      spatial_unit, age, nat, sex, sum_imm_sub_n = sum_imm_n, 
      total_imm_sub_n = total_imm_n, imm_sub_share = imm_share
    ),
  subregional_emi_share |> 
    dplyr::select(
      spatial_unit, age, nat, sex, sum_emi_sub_n = sum_imm_n, 
...
#   mutate(# aggregate 100+ year old people
...
#   group_by(age_group, nat, sex) |> 
#   summarise(mig_sub_sum = sum(sum_mig_sub_n)) |> 
#   View()

```

In the next step, we calculate the median net migration across four years. For that,

```{r, eval = TRUE}
# TODO review calculation
subregional_net_migration_median <- subregional_net_migration |> 
  dplyr::group_by(spatial_unit, age, sex, nat) |>
  dplyr::summarise(
    mig_sub_median = median(sum_mig_sub_n),
    imm_sub_median = median(sum_imm_sub_n),
    .groups = "drop"
  ) |>
  dplyr::mutate(sum_imm_sub_median = sum(imm_sub_median, na.rm = TRUE)) |>
  dplyr::ungroup() |>
  # surplus in migration is distributed according to share from total
  # immigration within the canton
  dplyr::mutate(
    mig_sub_share = imm_sub_median / sum_imm_sub_median,
    mig_sub = mig_sub_median - sum(mig_sub_median, na.rm = TRUE) * mig_sub_share
  )
...
# 
# subregional_net_migration_median |> 
#   mutate(# aggregate 100+ year old people
#         age = case_when(age > 100 ~ 100, .default = age),
#         # create age groups
#         age_group = cut(
...
Now we can add `mig_sub` as a column to the parameters.

```{r, eval = TRUE}
parameters_net <- fso_parameters_sub |> 
  dplyr::left_join(
    subregional_net_migration_median |> 
...
propop(
  ...
  fert_last = 50,
  share_born_female = 100 / 205,
  population = df_population_shares,
  binational = TRUE,
  subregional = "net"
)
```
