...
# Immigration records for three years
df_hist_imm_2023 <- tibble::tibble(
...
df_hist_imm <- rbind(df_hist_imm_2023, df_hist_imm_2024, df_hist_imm_2025)
...
#   head(100) |>
#   dplyr::mutate(across(
#     c("int_mothers":"imm_nat_n_distr"), \(x) sprintf(fmt = "%.3f", x)
#   )) |>
# DT::datatable()
...
fso_parameters_sub_distr_hist |>
  ...
  dplyr::mutate(share_imm_int = round(share_imm_int, 3),
                imm_int_n_distr = round(imm_int_n_distr, 3),
                share_imm_nat = round(share_imm_int, 3),
                imm_nat_n_distr = round(imm_nat_n_distr, 3)
                ) |>
  dplyr::arrange(year, age, sex, nat) |> 
  dplyr::filter(sex == "m" & nat == "int" & age == "29") |>
  dplyr::slice_head(n = 5) |> 
