...
It discusses **two key challenges** that arise when conducting such projections 
and presents strategies for addressing them:  
...
- For adjusting other rates, the procedure described in section [Migration between subregions](#between-subregions) could be useful.  
...
The data frame includes two columns with historic immigration from abroad (`hist_imm_int_n`) 
and from other cantons (`hist_imm_nat_n`) to the five regions and from four past years (2022 to 2025). The table below includes 40-year old Swiss women as a sample group:
...
to compute shares for custom age groups.       
...
`hist_imm_nat` or `hist_imm_int` as calculated above). 
...
`propop::calculate_shares()` computes the 
...
group and spatial unit is calculated</span> (`imm_share =  imm_n / sum_imm_n`). 

This table shows the result for 0-4 year old Swiss girls:
...
# Calculate the historic shares for immigration from other cantons
...
fso_parameters_sub_distr_hist <- fso_parameters_sub |>
  dplyr::left_join(
    data_distr_hist_int |>
      dplyr::select(spatial_unit, nat, sex, age, share_imm_int = imm_share),
    by = dplyr::join_by(spatial_unit, nat, sex, age)
  ) |>
  dplyr::left_join(
    data_distr_hist_nat |>
      dplyr::select(spatial_unit, nat, sex, age, share_imm_nat = imm_share),
    by = dplyr::join_by(spatial_unit, nat, sex, age)
  ) |>
...
the parameters (for simplicity, let's re-use `fso_parameters_sub_distr_hist`,
which was created for five different subregions but will nevertheless allow us
...
> To obtain such data you may summarize population register records as we described in the
...
Then, we compute the net migration by combining the historic shares and subtracting the aggregated subregional emigration (`hist_emi_sub_n`) from subregional immigration (`hist_imm_sub_n`).
