...
expressed as "number of people".   

If you want to adjust birth rates, you may find the package [`propopbirth`](https://github.com/statistik-aargau/propopbirth) useful. To adjust
other rates, check the next two sections. Each shows a possibility to distribute 
the cantonal "number of people" estimates among subregions.
...
The following examples explain how to adjust the population size in each subregion 
according to past migration.

## Subregional net migration

If you want to use **net migration numbers**, the column `mig_sub` is required in the 
...
If you prefer working with **migration rates**, columns for the emigration rate 
(`emi_sub`) and for the immigration rate (`imm_sub`) is needed.

Here we add `emi_sub` and `imm_sub` as *fictitious* parameters in the parameter input file. 
