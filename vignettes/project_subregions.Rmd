...
municipalities) is straightforward for data expressed as rates (e.g. mortality rate).
...
subregions is to determine each subregion's population size relative to the canton's total population and distribute the numbers accordingly. To put it somewhat simplistically, if a municipality represents 10% of the canton's population, it should proportionally receive 10% of the canton's incoming immigrants. The approach described below is more sophisticated in that it uses 
...
Input parameters (the rates and numbers for the 
subregions are so far identical to those for the whole canton): 
...
```{r data-population}
# Generate 5 random cuts to distribute original population; avoid extreme
# values
cut_1 <- {set.seed(1); round(runif(1, min = 0.1, max = 0.5), digits = 2)}
cut_2 <- {set.seed(2); round(runif(1, min = 0.1, max = 0.5), digits = 2)}
cut_3 <- {set.seed(3); round(runif(1, min = 0.1, max = 0.5), digits = 2)}
cut_4 <- {set.seed(4); round(runif(1, min = 0.1, max = 0.5), digits = 2)}
...
the identical for both types of immigration.
...
illustrate the suggested procedure in cases where zero observations exist for 
demographic groups.
...
The fictitious data assumes that we already performed this step. 
...
from abroad (` hist_imm_nat`) and other cantons (`hist_imm_nat`), aggregated
...
age group to a 5-year and a 10-year age group (e.g., 2-year olds become part of the age_groups 0-4 and 0-9), sums up all historical observations within the respective 5-year and 10-year 
age group (e.g., 0-4 and 0-9 year old Swiss males). These sums (`sum_5`and `sum_10`) are then divided 
by five or ten, respectively, and proportionally assigned to each member of the age group 
(`prop_5` and `prop_10`). 
...
group (e.g., the 76-year olds Swiss males in spatial unit 1), the mean share of 
the corresponding 5-year age group is used (i.e., 70-79-year olds in the case of 
76-year olds). If the share across 5-year age groups is also zero (e.g. 0-4 year old Swiss males in spatial unit 1), the 10-year age group is used (i.e. 0-9-year olds). In this case. the 10-year age_group is used for both 5-year age groups within the 10-year age group (i.e. 0-4 and 5-9 year olds) The variable `use_group` indicates which share is to be used. 
...
spatial unit. (`share =  use_share / use_share_sum`).
...
Note that the number of people projected for the whole canton 
is equal to the previous approach (projected population size (2020) = 686,746).
...
the column `mig_sub` (= migration in subregions) is required in the parameter data frame.
...
parameters_sub_mig <- fso_parameters_sub_distr_hist |>
  ...
  dplyr::select(
    nat, sex, age, year, scen, spatial_unit, birthrate, int_mothers,
...
*The procedure works likewise for the intercantonal immigration. For subregional migration saldos (e.g. movements between municipalities/districts) the function `calculate_shares()` is applied twice: to calculate subregional immigration shares and secondly, emigration shares. The column of interest is selected by the function argument `column_migration =` of `calculate_shares()`*

# Draft: Next steps
-   **Done: Historical shares:** refinement for groups with zero historical observations; e.g. using not only 1-year and 10-year groups but also 5-year groups or even more elaborate age groups such as life stages.
