...
The following two sections illustrate how to use the rate and net approach to  
account for migration between subregions.
...
based on `emi_sub`.    
1. This total number of people is summed up.  
1. This sum is then distributed among the locations using `imm_sub`. 
...
```{r diagram-rate-method-propop, results='asis', echo=FALSE}
...
data1 [label = 'Past migration\n= individual\n migration\n events', shape = folder, fillcolor = gold]
# data2 [label = 'Dataset 2', shape = folder, fillcolor = gold]

calc_emi [label =  'calc_\nemi_\nmean']

data2 [label = 'emi_sub\n= average\n emigration\n per group', shape = folder, fillcolor = gold]

calc_hist [label =  'calc_\nhist_\n_shares']

data3 [label = 'imm_sub\n= immigration\n share per group', shape = folder, fillcolor = gold]

propop [label =  'propop']


result [label= 'Result', shape = folder, fillcolor = gold]
...
To illustrate the rate approach, we add `emi_sub` and `imm_sub` as fictitious parameters in the parameter input file. 
