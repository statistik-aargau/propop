...
age group), sums up all historical observations within the respective 10-year 
age group (e.g., 0-9 year old Swiss males). This sum (`sum_10`) is then divided 
by ten and proportionally assigned to each member of the 10-year age group 
(`prop_10`). 
...
data_distr_hist_nat <- df_hist_imm |>
  ...
  dplyr::select(-c(
    "hist_imm_nat", "hist_imm_int", "age_group_10", "sum_10",
...
*The procedure works likewise for the intercantonal immigration. For subregional migration saldos (e.g. movements between municipalities/districts) the function `calculate_shares()` is applied twice: to calculate subregional immigration shares and secondly, emigration shares.*
