...
projection_canton_2030 |>
  # round to 2 digits
  dplyr::mutate(across(n:n_1, \(x) sprintf(fmt = "%.0f", x))) |>
...
The **output** file includes the end-of year
<span style="color:#007AB8;">number of people</span> (`n`)
per demographic group for the base year (= starting population) and the 
projected years (= projection). 
The <span style="color:#96D4FF;">components</span> that are used to project the 
<span style="color:#ffcc8f;">population at the end of the next year</span> (`n_1`), 
when the same group is one year older, are also included in the output. Thus, 
all elements of the cohort component equation are directly available from the
output.
