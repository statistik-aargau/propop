...
summary statistics **aggregated across observations** (e.g., all years, 
...
anymore; i.e., this code will fail from 2025 onwards if you try it for the years 
2019-2050):
...
#|   between FSO projection and result from propop projection for three
#|   age classes (0-19 years, 20-64 years, 65 years and older).
#|   The largest difference is smaller than 0.002%.
...
ggplot(fso_propop) +
  geom_jitter(aes(x = year, y = pe, color = age), size = 0.8) +
  scale_color_manual(values = c("age_00_19" = "#ffa81f", "age_20_64" = "#96D4FF", "age_65_plus" = "#A05388")) +
  labs(
    title = "Differences between FSO and propop projections for each demographic group (2025-2055)",
    x = "Year",
...
In the example data, the differences are as much as 30 person per demographic 
group and year. The main reason for these differences is that FSO uses a different 
starting population, which mainly differs for newborns and old people.

When using the exact same starting population, which is not publicly available, 
the largest difference between the FSO projections and those from {propop} is 
just one person per demographic group and year (across all 404 demographic groups 
and 31 years). 
